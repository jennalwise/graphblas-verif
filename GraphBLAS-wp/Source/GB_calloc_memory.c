...
/*@
 ...
 behavior inputs_valid :
    ...
    ensures (\result != \null ==>
                \valid(((char*)\result) + (0..max((size_t)1,nitems)*max((size_t)1,size_of_item)-1))     && // fresh not supported by WP/EVA, so need this redundant stmt for rte
                \initialized(((char*)\result)+(0..max((size_t)1,nitems)*max((size_t)1,size_of_item)-1)) &&
                \subset(((char*)\result)[0..max((size_t)1,nitems)*max((size_t)1,size_of_item)-1], {0})
            ) ;
