...
// *** JENNA CHANGE 7/13/18 ***
// Commented out built-in semirings monoids, because it is not needed for bfs
// verification. Only commented out semirings and monoids, because semirings included an extra file
// which is unneeded and monoids used INFINITY which is not supported by Frama-C (can be fixed if needed).
// Only using BinaryOp built-ins currently.
